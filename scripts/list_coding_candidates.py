#!/usr/bin/env python3
"""List non-solved rows from README.md Must-Do Syllabus (BFE coding problems)."""
...
def parse_rows() -> list[dict]:
    ...
    for line in README.read_text(encoding="utf-8").splitlines():
        if line.strip() == "## Must-Do Syllabus (Frontend Interview)":
            in_syllabus = True
            continue
        if in_syllabus and line.startswith("## ") and "Must-Do" not in line:
            break
        if not in_syllabus:
            continue
...
def main() -> None:
    ...
    if args.format == "json":
        ...
        sys.stdout.write("\n")
        return
    print(f"Coding candidates: **{len(rows)}** rows (not ✓). File: `{README}` (Must-Do Syllabus)\n")
